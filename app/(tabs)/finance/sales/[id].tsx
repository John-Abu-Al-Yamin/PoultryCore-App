...
const SaleDetailPage = () => {
  ...
  const saleDetails = sale?.data?.data;
  const totalPrice = Number(saleDetails?.total_price || 0);
  const paidAmount = Number(saleDetails?.paid_amount || 0);
  const remainingAmount = totalPrice - paidAmount;

  const handleDelete = () => {
    deleteSale(
      { id: id as string, url: `${endPoints.sales}/${id}` },
      {
        onSuccess: () => {
  ...
  const status =
    statusConfig[saleDetails?.status || ""] || statusConfig.unpaid;
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row items-start justify-between mb-6">
          <View className="flex-1">
            ...
            >
              رقم العملية #{saleDetails?.id}
            </AppText>
            <AppText variant="h1" className="mb-2">
              {saleDetails?.item_name}
            </AppText>
            <View className="flex-row items-center gap-3">
              ...
              <View className="flex-row items-center gap-1.5">
                ...
                <AppText variant="caption" muted>
                  {formatDate(saleDetails?.sale_date || "")}
                </AppText>
              </View>
            </View>
          </View>
          ...
          <View className="flex-row gap-2">
            <TouchableOpacity
              ...
              disabled={saleDetails?.status === "paid"}
            ...
            <TouchableOpacity
              onPress={() =>
                router.push(`/finance/sales/edit/${id}` as any)
              }
              disabled={saleDetails?.status === "paid"}
              className={`w-11 h-11 rounded-2xl items-center justify-center ${
                saleDetails?.status === "paid"
        ...
        {/* Customer Section */}
        <SectionHeader title="معلومات العميل" icon={Users} />
        <Card className="p-4 mb-6">
          <View className="flex-row items-center justify-between mb-4">
            <View className="flex-row items-center gap-3">
              <View className="w-12 h-12 rounded-full bg-primary-light/5 dark:bg-primary-dark/5 items-center justify-center">
                <AppText className="text-xl font-bold text-primary-light dark:text-primary-dark">
                  {saleDetails?.customer?.name?.charAt(0).toUpperCase() ||
                    "C"}
                </AppText>
              </View>
              <View>
            ...
            <TouchableOpacity
              onPress={() =>
                router.push(
                  `/more/customers/${saleDetails?.customer_id}` as any,
                )
              }
          ...
          {saleDetails?.customer?.phone && (
            <TouchableOpacity
              onPress={() =>
                Linking.openURL(`tel:${saleDetails.customer.phone}`)
              }
        ...
        <Card className="p-4 mb-6">
          <InfoRow
            ...
            value={saleDetails?.batch?.poultry_type || "---"}
            icon={Layers}
          ...
          <InfoRow
            label="سعر الوحدة"
            value={`${Number(saleDetails?.unit_price || 0).toLocaleString()} ج.م`}
            icon={DollarSign}
          />
          <InfoRow
            label="طريقة الدفع"
            value={
              paymentTypeLabels[saleDetails?.payment_type || ""] ||
        ...
        {saleDetails?.payments && saleDetails.payments.length > 0 ? (
          <View className="gap-3">
            {saleDetails.payments.map((payment: any, index: number) => (
              <Card
                key={payment.id}
