...
import { ContactRound, MapPin, Phone, User, ChevronRight } from "lucide-react-native";
...
export default function EditCustomerPage() {
  ...
  const onSubmit = (formData: CustomerFormData) => {
    mutate(
      ...
      {
        onSuccess: () => {
          toast.success("اتحدثت بيانات العميل بنجاح");
          router.back();
        },
        onError: (error: any) => {
          const errorMessage = error?.response?.data?.message || "حصل خطأ في تحديث بيانات العميل";
  ...
  if (isFetching) {
    return (
      <AppScreen scrollable={false}>
        <AppLoading fullScreen message="بيت حمّل بيانات العميل..." />
      </AppScreen>
    );
  }
    ...
    return (
      <AppScreen scrollable={false}>
        <AppError
          ...
          message="مقدرناش نوصل لـ بيانات العميل المطلوبة."
          onRetry={refetch}
          onBack={() => router.back()}
        />
      </AppScreen>
